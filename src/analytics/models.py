...
class ObjectViwedQuerySet(models.query.QuerySet):
...
class ObjectViewedManager(models.Manager):
    def get_queryset(self):
        return ObjectViwedQuerySet(self.model, using=self._db)
...
class UserSession(models.Model):
    ...
    def end_session(self):
        ...
        ended = self.ended
        try:
            Session.object.get(pk=session_key).delete()
            self.active = False
            self.ended = True
            self.save()
        except:
...
def post_save_session_receiver(sender, instance, created, *args, **kwags):
    if created:
