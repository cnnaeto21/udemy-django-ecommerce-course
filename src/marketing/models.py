...
def marketing_pref_update_receiver(sender, instance, created, *args, **kargs):
    if created:
...
def make_marketing_pref_receiver(sender, instance, created, *args, **kargs):
    if created:
        MarketingPrefrence.objects.get_or_create(user=instance)
